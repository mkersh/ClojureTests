...
(ns project-euler.euler8
  (:require [project-euler.support :as supp]
            [clojure.string :as str]))

(def the-1000-digit-number
...
(defn max-adjacent-product [num-adjacent]
  (let [search-str (str/replace the-1000-digit-number #"\n" "")
        search-str-len (count search-str)
        _ (prn "search-str-len" search-str-len)]
    (loop [start-pos 0 largest-prod 0]
      (let [end-pos (+ start-pos num-adjacent)
            eof? (> end-pos search-str-len)]
        (if eof?
          largest-prod
...
(comment
  ...
  ;; Answer=5832 (as given in the question)
  (supp/start-task :job-pp1  (fn [] (time (max-adjacent-product 4))))
  ;; Answer=23514624000
  (supp/start-task :job-pp1  (fn [] (time (max-adjacent-product 13))))
